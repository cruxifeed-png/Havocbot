...
def detect_signal(df):
    ...
    # SELL: EMA9 crosses below EMA21 + lower highs
    if ema9[-2] > ema21[-2] and ema9[-1] < ema21[-1]:
        if close[-1] < close[-2] < close[-3]:
            return "SELL"
    # BUY: EMA9 crosses above EMA21 + higher lows
    if ema9[-2] < ema21[-2] and ema9[-1] > ema21[-1]:
        if close[-1] > close[-2] > close[-3]:
            return "BUY"
    return None
...
@app.route("/")
def home():
    return "CruxiFX Bot is running!"

# Run bot in a separate thread so Flask stays alive
